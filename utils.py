...
def add_mask2image_binary_fg(images_path, masks_path, masked_path):
...
def add_mask2image_binary_bg(images_path, masks_path, masked_path):
# Add binary masks to images
    for img_item in os.listdir(images_path):
        #print(img_item)
        img_path = os.path.join(images_path, img_item)
        ...
        mask_path = os.path.join(masks_path, img_item[:-4]+'.png')  # mask是.png格式的，image是.jpg格式的
        mask = cv2.imread(mask_path, cv2.IMREAD_GRAYSCALE)  # 将彩色mask以二值图像形式读取
        masked = cv2.add(img, np.zeros(np.shape(img), dtype=np.uint8), mask=38-mask)  #将image的相素值和mask像素值相加得到结果
        cv2.imwrite(os.path.join(masked_path, img_item), masked)
...
def input_setup(sess,config,data_dir,index=0):
  ...
  if config.is_train:
    #取到所有的原始图片的地址
    data = add_mask2image_binary_fg("Train_ir", "Train_mask", "Train_ir_fg")
    data = add_mask2image_binary_fg("Train_vi", "Train_mask", "Train_vi_fg")
    data = add_mask2image_binary_fg("Test_ir", "Test_mask", "Test_ir_fg")
    data = add_mask2image_binary_fg("Test_vi", "Test_mask", "Test_vi_fg")

    data = add_mask2image_binary_bg("Train_ir", "Train_mask", "Train_ir_bg")
    data = add_mask2image_binary_bg("Train_vi", "Train_mask", "Train_vi_bg")
    data = add_mask2image_binary_bg("Test_ir", "Test_mask", "Test_ir_bg")
    data = add_mask2image_binary_bg("Test_vi", "Test_mask", "Test_vi_bg")
  else:
    data = add_mask2image_binary_fg("Train_ir", "Train_mask", "Train_ir_fg")
    data = add_mask2image_binary_fg("Train_vi", "Train_mask", "Train_vi_fg")
    data = add_mask2image_binary_fg("Test_ir", "Test_mask", "Test_ir_fg")
